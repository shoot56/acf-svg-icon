...
if (!class_exists('swp_acf_field_svg_icon')) {
    class swp_acf_field_svg_icon extends acf_field
    {
        ...
        public function parse_svg_sprite($file_path = '')
        {
            ...
            // Если файл передан без пути, используем стандартный путь
    $file_path = $file_path ? $file_path : '/images/icons.svg';

    // Проверяем, есть ли активная дочерняя тема
    $theme_file_path = is_child_theme() ? get_stylesheet_directory() . $file_path : get_template_directory() . $file_path;

    // Если файла в дочерней теме нет, пытаемся загрузить из родительской
    if (!file_exists($theme_file_path)) {
        $theme_file_path = get_template_directory() . $file_path;
    }

    // Если файла нет и в родительской теме, возвращаем пустой массив
    if (!file_exists($theme_file_path)) {
        return array();
    }
...
//            set_transient($cachekey, $icons, YEAR_IN_SECONDS);
        ...
        function svg_files_as_choices($field)
        {
            ...
            $default_path = $field["default_path"] ?? '/images/';
            
            if (is_child_theme()) {
                $dir = get_stylesheet_directory() . '/' . ltrim($default_path, '/');
                if (is_dir($dir)) {
                    $files = scandir($dir);
                    foreach ($files as $file) {
                        if (pathinfo($file, PATHINFO_EXTENSION) === 'svg') {
                            $svg_files['/' . $default_path . '/' . $file] = str_replace('.svg', '', $file);
                        }
                    }
                }
            }

            $dir = get_template_directory() . '/' . ltrim($default_path, '/');
            if (is_dir($dir)) {
                $files = scandir($dir);
                foreach ($files as $file) {
                    if (pathinfo($file, PATHINFO_EXTENSION) === 'svg') {
                        if (!isset($svg_files['/' . $default_path . '/' . $file])) {
                            $svg_files['/' . $default_path . '/' . $file] = str_replace('.svg', '', $file);
                        }
                    }
                }
